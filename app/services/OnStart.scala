...
class OnStartImpl @Inject() (appLifecycle: ApplicationLifecycle) extends OnStart {
  ...
  def createProducts(): List[Product] = {
    ...
    products = Product.saveOrUpdate(Product(ProductCreate("Coca Cola", Option("https://walmartar.vteximg.com.br/arquivos/ids/842749-1000-1000/Mini-Coronitas-Chocolate-140gr-1321.jpg?v=636839613155800000"), "Caja de refresco Coca Cola en 24 botellas de 600 cada una de Coca-Cola.\nEs una deliciosa bebida refrescante y revitalizante, ayudara para quitar la sed además de su tradicional sabor.", isValidated = true, 1, 3))).get :: products
    ...
    products = Product.saveOrUpdate(Product(ProductCreate("Samsung LED 32", Option("https://http2.mlstatic.com/tv-led-samsung-32-fullhd-1080p-hdmi-audio-optico-outlet-D_NQ_NP_795522-MLA27725271384_072018-Q.jpg"), "Descubre una nueva manera de ver televisión con el TV 32J4300 de Samsung. Este innovador televisor inteligente de 32 pulgadas te brindará imágenes increíbles y podrás disfrutar de tus programas o películas favoritas en una gran pantalla. No esperes más para adquirir este televisor HD solo en Carrefour", isValidated = true, 1, 8))).get :: products
    products = Product.saveOrUpdate(Product(ProductCreate("Equipo de Musica Noblex", Option("https://http2.mlstatic.com/equipo-de-musica-noblex-mnx-150-bt-D_NQ_NP_601808-MLA30083669600_042019-Q.jpg"), "El equipo de audio Noblex Mnx121 es un completo sistema de sonido de alta definición que te brinda nada menos que 1500W de potencia mediante dos parlantes. Para tu comodidad incorpora un control remoto con amplias funciones, y un lector de tarjetas de memoria para que puedas compartir tus archivos de sonido de forma práctica y sencilla. ", isValidated = true, 1, 9))).get :: products
    products
  }
